...
pub fn main() !void {
    ...
    try sample_model.round(allocator);
    try sample_model.round(allocator);
    try sample_model.round(allocator);
    try sample_model.round(allocator);
    try sample_model.round(allocator);
    ...
    try sample_model.round(allocator);
    try sample_model.round(allocator);
    std.debug.print("{d}\n", .{sample_model.calculateEmpties()});

    var model = Model(70, 70).fromInput(input_str);
    try model.round(allocator);
    try model.round(allocator);
    try model.round(allocator);
    try model.round(allocator);
    try model.round(allocator);
...
fn Model(comptime width: u8, comptime height: u8) type {
    return struct {
        ...
        fn round(self: *Self, allocator: std.mem.Allocator) !void {
