...
fn part2(input_str: []const u8) u24 {
    var product: u24 = 1;

    var iter = std.mem.tokenize(u8, input_str, "\n");
    var x: u8 = 0;
    while (x < 3) : (x += 1) {
        const val = Model.fromString(iter.next().?).getQualityLevel();
        std.debug.print("{d}\n", .{val});
...
const Model = struct {
    ...
    fn getQualityLevel(self: Model) u16 {
        ...
        var model: Model = self;
        if (model.ore_stat.production < model.ore_cost or
            model.ore_stat.production < model.clay_cost or
            model.ore_stat.production < model.obsidian_cost[0] or
            model.ore_stat.production < model.geode_cost[0])
        {
            model.buildOreBot();
        ...
        model = self;
        if (model.clay_stat.production < model.obsidian_cost[1]) {
            model.buildClayBot();
            m = std.math.max(m, model.getQualityLevel());
        }

        model = self;
        if (model.obsidian_stat.production < model.geode_cost[1]) {
