    ...
    fn getMaxFlow(self: Model, allocator: std.mem.Allocator) !u16 {
        ...
        while (frontier.readItem()) |state| {
            ...
            while (id_0 < len) : (id_0 += 1) {
                const mask = @as(u16, 1) << id_0;

                if (state.mask & mask == 0) {
                    var distance = distances.items[@as(u16, len) * state.id_0 + id_0];
                    if (distance < state.time_0) {
                        var next_state = state;
                        next_state.id_0 = id_0;
                        next_state.mask |= mask;
                        next_state.time_0 -= distance + 1;

                        const flow_rate = self.hash_map.get(self.key_valves.items[id_0]).?.flow_rate;
                        var flow: u16 = reached.get(state).?;
                        flow += @as(u16, next_state.time_0) * flow_rate;
            ...
            while (id_1 < len) : (id_1 += 1) {
                const mask = @as(u16, 1) << id_1;

                if (state.mask & mask == 0) {
                    var distance = distances.items[@as(u16, len) * state.id_1 + id_1];
                    if (distance < state.time_1) {
                        var next_state = state;
                        next_state.id_1 = id_1;
                        next_state.mask |= mask;
                        next_state.time_1 -= distance + 1;

                        const flow_rate = self.hash_map.get(self.key_valves.items[id_1]).?.flow_rate;
                        var flow: u16 = reached.get(state).?;
                        flow += @as(u16, next_state.time_1) * flow_rate;
